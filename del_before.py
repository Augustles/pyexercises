...
for file in files:
    ...
    l_time = datetime.datetime.fromtimestamp(time.mktime(l_time)) # 处理为datetime
    c_time = time.ctime(os.path.getctime(file)) # 获取文件创建时间str
    c_time = datetime.datetime.strptime(c_time, '%a %b %d %H:%M:%S %Y') # 转化为datetime
    # c_time = time.strptime(c_time, '%a %b %d %H:%M:%S %Y') # 转为stract_time
