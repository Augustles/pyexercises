...
# do 一个用来声明一个循环，这个循环的结束条件可以通过while关键字设置 
# double 一个用来定义一个double类型的变量 
# else 一个如果if语句的条件不满足就会执行该语句。 
# final 
# 你只能定义一个实体一次，以后不能改变它或继承它。更严格的讲：一个final修饰的类不能被子类化，一个final修饰的方法不能被重写，一个final修饰的变量不能改变其初始值。 
# finally 
# 一个用来执行一段代码不管在前面定义的try语句中是否有异常或运行时错误发生。 
# float 一个用来定义一个浮点数变量 
# for 
# 一个用来声明一个循环。程序员可以指定要循环的语句，推出条件和初始化变量。 
# if 
# 用来生成一个条件测试，如果条件为真，就执行if下的语句。 
# implements 
# 在类的声明中是可选的，用来指明当前类实现的接口。 
# import 
